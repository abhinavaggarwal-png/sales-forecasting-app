...
# Title
st.markdown('<div class="main-header">🎯 Sales Forecasting Dashboard - November 2025</div>', 
            unsafe_allow_html=True)
st.markdown("---")
...
with st.expander("🔧 Advanced: City x SKU Level Controls", expanded=False):
    st.warning("⚠️ Advanced mode: Set individual controls for specific city-SKU combinations")
...
st.markdown("---")
if st.button("🚀 Generate November Forecast", type="primary", use_container_width=True):
    with st.spinner("Generating forecast with confidence intervals..."):
        try:
            # Step 1: Prepare November baseline (no changes)
            ...
            # Step 2: If no changes, use baseline for both
            if discount_change == 0 and budget_change == 0:
                predictions = predictions_baseline
            else:
                # Apply changes to October data
                ...
                # Prepare November features with changes
            ...
            # Store results
            st.session_state.df_nov = df_nov_baseline  # Always store baseline structure
...
if st.session_state.predictions_generated:
    st.markdown("---")
    st.header("📊 Forecast Results - November 2025")
    ...
    # Row 1: Forecast numbers (2 large cards)
    col1, col2 = st.columns(2)
    ...
    # Row 2: Confidence Intervals (3 cards)
    col3, col4, col5 = st.columns(3)
    
    with col3:
        ci_50_l = st.session_state.predictions['aggregate_ci_50_lower']
        ci_50_u = st.session_state.predictions['aggregate_ci_50_upper']
        st.metric(
            "50% Confidence Interval",
            f"{ci_50_l/1000:.0f}K - {ci_50_u/1000:.0f}K",
            help="50% chance total forecast falls in this range"
        )
    
    with col4:
        ci_75_l = st.session_state.predictions['aggregate_ci_75_lower']
        ci_75_u = st.session_state.predictions['aggregate_ci_75_upper']
        st.metric(
            "75% Confidence Interval",
            f"{ci_75_l/1000:.0f}K - {ci_75_u/1000:.0f}K",
            help="75% chance total forecast falls in this range"
        )
    
    with col5:
        ci_95_l = st.session_state.predictions['aggregate_ci_95_lower']
        ci_95_u = st.session_state.predictions['aggregate_ci_95_upper']
        st.metric(
            "95% Confidence Interval",
            f"{ci_95_l/1000:.0f}K - {ci_95_u/1000:.0f}K",
            help="95% chance total forecast falls in this range"
        )
    st.markdown("---")
